...
void loop() {
  ...
  if(fifo2Count < packet2Size){
          //Lets go back and wait for another interrupt. We shouldn't be here, we got an interrupt from another event
      // This is blocking so don't do it   while (fifoCount < packetSize) fifoCount = mpu.getFIFOCount();
  }
  else if ((mpu2IntStatus & (0x01 << MPU6050_INTERRUPT_FIFO_OFLOW_BIT)) || fifo2Count >= 1024) {
    // reset so we can continue cleanly
    mpu2.resetFIFO();
    //  fifoCount = mpu.getFIFOCount();  // will be zero after reset no need to ask
    Serial.println(F("FIFO overflow!"));
  // otherwise, check for DMP data ready interrupt (this should happen frequently)
  } else if (mpu2IntStatus & (0x01 << MPU6050_INTERRUPT_DMP_INT_BIT)) {
      while(fifo2Count >= packet2Size) { // Lets catch up to NOW, someone is using the dreaded delay()!
